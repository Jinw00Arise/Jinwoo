...
import "github.com/Jinw00Arise/Jinwoo/pkg/maple"

// EncodeHeader creates the 4-byte header: version XOR'd with IV[2:3], then length XOR'd with that.
func EncodeHeader(length int, iv []byte) []byte {
	header := make([]byte, 4)
	header[0] = byte(maple.SendVersion&0xFF) ^ iv[2]
	header[1] = byte(maple.SendVersion>>8) ^ iv[3]
